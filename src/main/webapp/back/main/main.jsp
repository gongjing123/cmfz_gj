...
<script type="text/javascript">
    ...
    $(function () {
        //页面加载完成之后显示菜单数据
        $.post("${pageContext.request.contextPath}/menus/findAll",function (menu) {
           ...
           // console.log(menu);
            ...
            $.each(menu,function (index,m) {
                ...
                $.each(m.list,function(idx,child){
                    content += "<a onclick=\"addTabs('"+child.name+"','"+child.iconCls+"','"+child.href+"')\" style='width:95%;margin:10px 0px; border: 2px #eec1be solid;' class='easyui-linkbutton' data-options=\"plain:true,iconCls:'"+child.iconCls+"'\">"+child.name+"</a><br>";
                });
                content += "</div>"
                //添加菜单
                $("#menu").accordion('add',{
    ...
    function editpassword() {
        ...
        $("#editpassword").dialog({
            href:'${pageContext.request.contextPath}/back/edit.jsp',
            buttons:[
                {
                    iconCls:'icon-save',
                    text:"修改",
...
<body class="easyui-layout">   
    <div data-options="region:'north',split:true" style="height:60px;background-color:  #5C160C">
    	...
    	<div style="font-size: 16px;color: #FAF7F7;font-family: 楷体;width: 300px;float:right;padding-top:15px">欢迎：${sessionScope.admin.name }
            <a href="#"  class="easyui-linkbutton"  onclick="editpassword()" data-options="iconCls:'icon-edit'">修改密码</a>&nbsp;
            <a href="${pageContext.request.contextPath}/Admin/out" class="easyui-linkbutton" onclick="quit()" data-options="iconCls:'icon-01'">退出系统</a></div>
