<%@page isELIgnored="false" pageEncoding="UTF-8" %>
    ...
    <script>
        $(function () {
            ...
            $("#articledg").datagrid({
                ...
                columns:[[
                    ...
                    {title:'content',field:'content',width:120,},
                    {title:'insertimgPath',field:'insertimgPath',width:120,sortable:true},
                    {title:'publishDate',field:'publishDate',width:120,sortable:true,

                    },
                    {title:'guru_name',field:'guru_name',width:120,},
                ...
                detailFormatter: function(rowIndex, rowData){
                    return '<table><tr>' +
                        '<td rowspan=2 style="border:0"><img src="${pageContext.request.contextPath}/back/' + rowData.insertimgPath + '" style="height:50px;"></td>' +
                        '<td style="border:0">' +
                        '<p>Attribute: ' + rowData.title + '</p>' +
                        '<p>Status: ' + rowData.guru_name + '</p>' +
                        '</td>' +
                        '</tr></table>';
                },
        ...
        //打开保存用户对话框函数
        function openSaveActicleDialog(){
            $("#saveActicleDialog").dialog({
                buttons:[{
                    iconCls:'icon-save',
                    text:'保存',
                    handler:function(){
                        //保存用户信息
                        $("#saveActicleInputForm").form('submit',{
                            ...
                            success:function(result){//响应的一定是json格式字符串   使用应该先转为js对象
                                ...
                                //关闭对话框
                                $("#saveActicleDialog").dialog('close');
                                //刷新datagrid
        ...
        function delBatchRows(){
            ...
            if(rows.length<=0){
                ...
                $.ajax({
                    url:"${pageContext.request.contextPath}/emp/delBatchRows",
                    type:"POST",
...
<%--datagrid工具栏--%>
<div id="articletb">
    <a href="#" class="easyui-linkbutton" onclick="openSaveActicleDialog();" data-options="iconCls:'icon-add',plain:true">添加</a>
    <a href="#" class="easyui-linkbutton" onclick="delBatchRows();" data-options="iconCls:'icon-remove',plain:true">批量删除</a>
</div>

<%--保存文章对话框--%>
<div id="saveActicleDialog"
     data-options="href:'${pageContext.request.contextPath}/back/article/save.jsp',
         draggable:true,iconCls:'icon-save',width:600,height:400,title:'保存文章信息'">

</div>
<%--更新文章对话框--%>
<div id="editActicleDialog" data-options="draggable:false,iconCls:'icon-edit',width:600,height:400,title:'更新文章信息'">
</div>
